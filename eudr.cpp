...
#define SCR_COLR_FIRST  15
#define SCR_COLR_COUNT  22

// remaster-only player colors
const u8 scrcolors[SCR_COLR_COUNT - SCR_COLR_FIRST][8] = {
{ 83, 153,  78, 150, 148, 144,  67,  65}, // 15 - pink (there is not actually any pink lmao)
{101,  97,  97,  92,  92,  89,  86, 138}, // 16 - olive
{166, 166, 106, 101,  97,  96,  16,  86}, // 17 - lime
{160,  42,  42,  42,  41, 118,  40, 138}, // 18 - navy
{164, 164, 164, 163, 161, 161,  90, 139}, // 19 - magenta
{ 74,  74,  73, 144, 142, 141, 140,  64}, // 20 - grey
{142,  67, 141,  66, 140, 139,  64, 138}  // 21 - black
};
...
u8 colorShuffle[SCR_COLR_COUNT] = {0};
...
SBmpLoadImage_type SBmpLoadImage;
...
__stdcall u32 CHK_CRGB(chk_call_data* thing, u32 size, u32 idk){
  ...
  u8 tunit[16][8];
  if(SBmpLoadImage("game\\tunit.pcx", NULL, (u8*)tunit, sizeof(tunit), NULL, NULL, NULL) == false){
    // could not load tunit.pcx
    return 1;
  }
  ...
  for(i = 0; i < 8; i++){
    if(crgbdata.mode[i] == CRGB_RANDOM){
      ...
      if(crgbdata.rgb[i][2] < SCR_COLR_COUNT){ // color in blue channel
        // exclude color from shuffle pool
        colorShuffle[crgbdata.rgb[i][2]] = 0;
      }
    }
  }
  ...
  for(i = 8; i > 0; ){
    ...
    switch(crgbdata.mode[i]){
      case CRGB_RANDOM:
        crgbdata.rgb[i][2] = colorShuffle[i]; // override color with shuffled color
        useCOLR = 1;
        break;
      case CRGB_RGB:
        genPCol(&unitColTable[i*8], crgbdata.rgb[i][0], crgbdata.rgb[i][1], crgbdata.rgb[i][2]);
        unkPlayerCol[i] = i;
        break;
      case CRGB_COLR:
        useCOLR = 1;
    ...
    if(useCOLR){
      if(crgbdata.rgb[i][2] >= SCR_COLR_COUNT){
        // undefined colors just use default
        crgbdata.rgb[i][2] = i;
      }
      if(crgbdata.rgb[i][2] < SCR_COLR_FIRST){ // use tunit.pcx
        if(crgbdata.rgb[i][2] == 14) crgbdata.rgb[i][2]++; // cyan is given an ID of 14 to skip the duplicate light yellow in tunit.pcx
        memcpy(&unitColTable[i*8], tunit[crgbdata.rgb[i][2]], 8);
      }else{ // use scr-only colors
        memcpy(&unitColTable[i*8], scrcolors[crgbdata.rgb[i][2] - SCR_COLR_FIRST], 8);
      }
      unkPlayerCol[i] = crgbdata.rgb[i][2];
    }
    
    miniColTable[i] = unitColTable[i*8]; // technically should read from tminimap.pcx
  }
...
u32 patch(){
  ...
  *(u32**)addr = (u32*)(&TRGACT_SetDeaths);
  ...
  SBmpLoadImage = (SBmpLoadImage_type)GetProcAddress(hStorm, (LPSTR)323);
